...
int compid = 1;
int serial_compid = 0;
std::string port = "/dev/ttyACM1";              ///< The serial port name, e.g. /dev/ttyUSB0
...
void* serial_wait(void* serial_ptr)
{
  ...
  while (1)
  {
    ...
    if (msgReceived)
    {
       ...
       */
      ros_mavlink::Mavlink rosmavlink_msg;
      ...
      rosmavlink_msg.checksum=message.checksum;
      rosmavlink_msg.magic = message.magic;
       ...
       */
     // printf("publishing message id: %d received from APM \n",message.msgid);
      ...
      switch (message.msgid)
      {
       ...
       case MAVLINK_MSG_ID_SYS_STATUS:
       {
    	  ...
    	  // printf("battery voltage: %d  remaining: %d \n",stat.voltage_battery,stat.battery_remaining);
       }
       break;
       ...
       case MAVLINK_MSG_ID_RC_CHANNELS_RAW:
       	{
       		...
       		if(rc_raw.chan7_raw > 1500){
       			printf("going backt to  radio control \n");
       		 mavlink_message_t m;
       			mavlink_rc_channels_override_t msg_struct;
       			msg_struct.chan1_raw=0;
       			msg_struct.chan2_raw=0;
       			msg_struct.chan4_raw=0;
       			msg_struct.chan5_raw=0;
       			msg_struct.chan6_raw=0;
       			msg_struct.chan7_raw=0;
       			msg_struct.chan8_raw=0;
       			msg_struct.chan3_raw=0;
       			msg_struct.target_system=sysid;
       			msg_struct.target_component=MAV_COMP_ID_ALL;
       			mavlink_msg_rc_channels_override_encode(255,MAV_COMP_ID_ALL,&m,&msg_struct);

       			ros_mavlink::Mavlink mavlink_msg;
       			    mavlink_msg.len = m.len;
       			    mavlink_msg.seq = m.seq;
       			    mavlink_msg.sysid = m.sysid;
       			    mavlink_msg.compid = m.compid;
       			    mavlink_msg.msgid = m.msgid;
       			    mavlink_msg.checksum = m.checksum;
       			    mavlink_msg.magic = m.magic;
       			    for (int i = 0; i < sizeof(m.payload64); i++)
       			      {
       			        (mavlink_msg.payload64).push_back(m.payload64[i]);
       			      }
       			temp_pub.publish(mavlink_msg);
       			radio_control=true;

       		}
       		else{
       			printf("in to program control\n");

       		}
       		printf("channel 7: %d  3: %d \n", rc_raw.chan7_raw,rc_raw.chan3_raw);
...
void mavlinkCallback(const ros_mavlink::Mavlink &mavlink_ros_msg)
{
  ...
  if(radio_control ){
	 ...
	 msg_struct.target_system=sysid;
	 msg_struct.target_component=MAV_COMP_ID_ALL;
	 mavlink_msg_rc_channels_override_encode(255,MAV_COMP_ID_ALL,&msg1,&msg_struct);
  ...
  msg1.msgid = mavlink_ros_msg.msgid;
  msg1.checksum=mavlink_ros_msg.checksum;
  msg1.compid = mavlink_ros_msg.compid;
  msg1.sysid = mavlink_ros_msg.sysid;
  msg1.magic = mavlink_ros_msg.magic;
  msg1.len = mavlink_ros_msg.len;
  msg1.seq  = mavlink_ros_msg.seq;
  for (int i = 0; i < sizeof(mavlink_ros_msg.payload64); i++)
        {
          msg1.payload64[i]=mavlink_ros_msg.payload64[i];
        }

   mavlink_finalize_message_chan(&msg1, mavlink_ros_msg.sysid, mavlink_ros_msg.compid, MAVLINK_COMM_0,
                                  mavlink_ros_msg.len, mavlink_crcs[msg1.msgid]);


//     	 printf("sending message id : %d to APM \n",msg1.msgid);
//     	 printf("sending system id : %d to APM \n ",msg1.sysid);
//     	 printf("sending component_id id : %d to APM \n  ",msg1.compid);
//     	 printf("sending len id : %d to APM \n ",msg1.len);
//     	 printf("sending checksum : %d to APM \n ",msg1.checksum);
//     	 printf("sending magic : %d to APM \n ",msg1.magic);
//     	 printf("sending seq : %d to APM \n ",msg1.seq);
//     	 mavlink_command_long_t temp;
//         mavlink_msg_command_long_decode(&msg1,&temp);
//         printf("command %d \n",temp.command);
//         printf("param 1 %f \n",temp.param1);
//         printf("confirmation %d \n",temp.confirmation);
//         printf("target comp %d \n",temp.target_component);
//         printf("target sys %d \n",temp.target_system);
   /**
  ...
  int messageLength = mavlink_msg_to_send_buffer(buffer, &msg1);
  if (debug)
...
//  	pr.target_system=1;
//  	mavlink_msg_param_request_list_encode(1,1,&msg,&pr);
...
//
//
//  	 printf("sending message id : %d to APM --- %d\n ",msg1.msgid,msg.msgid);
//  	  	   printf("sending system id : %d to APM --- %d\n ",msg1.sysid,msg.sysid);
//  	  	   printf("sending component_id id : %d to APM --- %d \n  ",msg1.compid,msg.compid);
//  	  	   printf("sending len id : %d to APM --- %d\n ",msg1.len,msg.len);
//  	  	   printf("sending checksum : %d to APM --- %d\n ",msg1.checksum,msg.checksum);
//  	  	   printf("sending magic : %d to APM --- %d\n ",msg1.magic,msg.magic);
//  	  	   printf("sending seq : %d to APM --- %d\n ",msg1.seq,msg.seq);


  tcflush(fd, TCOFLUSH);
  if (messageLength != written){
    fprintf(stderr, "ERROR: Wrote %d bytes but should have written %d\n", written, messageLength);
    printf("error in message length");
  }
}

int main(int argc, char **argv)
{
  ros::init(argc, argv, "mavlink_ros_node");
  ...
  mavlink_sub = mavlink_nh.subscribe("mavlink/to", 1000, mavlinkCallback);
  mavlink_pub = mavlink_nh.advertise<ros_mavlink::Mavlink>("mavlink/from", 1000);
  temp_pub = mavlink_nh.advertise<ros_mavlink::Mavlink>("mavlink/to",1000);
